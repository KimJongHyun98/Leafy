...
<head>
<meta charset="UTF-8">
<title>문의사항 리스트 페이지</title>
<script
	src="https://ajax.googleapis.com/ajax/libs/jquery/3.6.0/jquery.min.js"></script>
<script>
$(function(){    
    $("#btnWrite").click(function(){
        location.href="mtmWrite.do";
    });
});
</script>
<style>
			...
			<div class="service_box">
				...
				<div class="service_content">
					...
					<table>
						...
						<c:forEach var="mtm" items="${requestScope.mtmlist}">
							<tr>
								<td>${mtm.mno}</td>
								<td>${mtm.id }</td>
								<td><a href="mtmView.do?mno=${mtm.mno }">${mtm.mtm_request_title}</a></td>
								<td>${mtm.mtm_request_date }</td>
								<td>${mtm.mtm_request_status }</td>
							</tr>
						</c:forEach>
						<!-- 페이징 처리 -->
						<tr>
							<td colspan="7"><c:if
									...
									<a
										href="mtmList.do?pageNo=${requestScope.pagging.startPageOfPageGroup-1 }">
										＜＜ </a>
						...
						${i }
										...
										<c:otherwise>
											<a href="mtmList.do?pageNo=${i} ">${i }</a>
										</c:otherwise>
									</c:choose>
								</c:forEach> <c:if test="${requestScope.pagging.nextPageGroup }">
									<a
										href="mtmList.do?pageNo=${requestScope.pagging.endPageOfPageGroup+1 }">
										＞＞ </a>
								</c:if></td>
						</tr>
